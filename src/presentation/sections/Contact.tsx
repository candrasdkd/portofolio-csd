...
const Contact: React.FC = () => {
  ...
  const sendEmail = (e: React.FormEvent) => {
    ...
    emailjs.sendForm('service_ap5quuk', 'template_f9tkkne', formRef.current, { publicKey: '6jyFeXZCNSYpqM0f4' })
      .then(() => {
        setStatus('success');
        setIsLoading(false);
    ...
    <section id="contact" className="relative py-24 px-4 md:px-8 bg-[#0f1115] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8 lg:pt-10">
            <div>
              <h2 className="text-sm font-semibold text-blue-500 tracking-wider uppercase mb-2">{t('contact.title')}</h2>
              <h3 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6 transition-colors">
                {t('contact.heading1')} <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">{t('contact.heading2')}</span>
              </h3>
              <p className="text-gray-600 dark:text-gray-400 text-lg leading-relaxed max-w-md transition-colors">{t('contact.subtitle')}</p>
            </div>

            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="p-3 bg-white/50 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/10 text-blue-500 transition-colors"><Mail size={24} /></div>
                <div>
              ...
              <div className="flex items-start gap-4">
                <div className="p-3 bg-white/50 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/10 text-purple-500 transition-colors"><MapPin size={24} /></div>
                <div>
            ...
            <div className="pt-8 border-t border-gray-200 dark:border-white/10 transition-colors">
              ...
              <div className="flex gap-4">
                <a href="https://github.com/candrasdkd" target="_blank" rel="noopener noreferrer" className="p-2 bg-white/5 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition-colors border border-white/5"><Github size={20} /></a>
                <a href="https://www.linkedin.com/in/candrasdk/" target="_blank" rel="noopener noreferrer" className="p-2 bg-white/5 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition-colors border border-white/5"><Linkedin size={20} /></a>
              </div>
            </div>
          </motion.div>
            ...
            <form ref={formRef} className="space-y-6" onSubmit={sendEmail}>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <textarea rows={4} name="message" required placeholder="..."
              ...
              <button type="submit" disabled={isLoading}
                className={`w-full py-4 px-6 font-bold rounded-xl transition-all flex items-center justify-center gap-2 group ${isLoading ? 'bg-gray-300 dark:bg-gray-800 cursor-not-allowed text-gray-500' : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white shadow-lg shadow-blue-500/25'}`}>
                {isLoading ? <>{t('contact.sending')} <Loader2 className="animate-spin" size={18} /></> : <>{t('contact.sendMsg')}<ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>}
              </button>
